 ...
 *
 */
extern const char psr_jpg_logo[8197];
extern const char mscc_jpg_logo[7778];
...
)
{
    ...
    if(err == ERR_OK)
    {
        ...
        if(buflen >= 5 &&
           ...
           buf[4] == '/' )
        {
        	if( !strncmp( buf, "GET /RTCdata", 12 ) || !strncmp( buf, "GET /status", 11 )||!strncmp( buf, "GET /trigger0?", 14)||!strncmp( buf, "GET /trigger1?", 14)|| !strncmp( buf, "GET /index_r1_c1", 16 )|| !strncmp( buf, "GET /psr_actel", 14 ))
            	 ...
            	 if( buf[5]=='s')
            	  {
					  ...
					  {
						  netconn_write(conn, psr_jpg_logo, sizeof(psr_jpg_logo)-1, NETCONN_NOCOPY);
					  }
					  else if(buf[5]=='t')
					  {
        	 ...
        	 else if( !strncmp( buf, "GET /LED", 8 ) )
					 {
						 ...
						 snprintf((char *)uip_appdata, 1200,
																	...
																	"<META content=\"MSHTML 6.00.2900.5726\" name=GENERATOR></HEAD>"
																	 ...
																	 "<p style=\"text-align: center; font-size:22px; color:#333333;font-weight: bold; \">Управление светодиодами </p>"
																//	 "<p style=\"text-align: center;font-size: 18px; color:  #003399;\">LEDs on the board should blink once from 1 to 255 <br>"
								 	 	 	 	 	 	 	 	 	 "<p style=\"text-align: center;font-size: 18px; color:  #003399;\">Введите число от 0 до 15 :"
						             	        		 			"<input type=\"text\" maxlength=3 name=\" INPUTSTRING \" />"
						             	        		 			"<input type=\"submit\" value=\"Зажечь!\" /></p><br>"
						             	        		 			"</form>"
						             	        		 			"<form>"
								                                    "<p style=\"text-align: center;\">"
                                                                     "<input type = \"Button\" value = \"На главную \" onclick = \"window.location.href='index.html'\"></p>"
						             	        		 			"</form>"
        		        ...
        		        else
        		        {
            	         ...
            	         netconn_write( conn, data_google_search_htm, sizeof( data_google_search_htm), NETCONN_COPY );
            	        }

						 else if( !strncmp( buf, "GET /corePWM.htm", 16) )
            	        	{
							 const static char data_Gadgets_html[]="<HTML>\
							 <head><title>SmartFusion2 corePWM</title></head>\
							 <body><table width=\"100%\" cellspacing=\"1\">\
							 <tr><td height=\"15\"></td></tr>\
							 <tr><td colspan=\"5\" align=\"center\" ><b><font color=\"#333333\"><font size=\"5\">SmartFusion2 corePWM Demonstration</font></font><b>\
							 </td></tr>\
            	        	...
            	        	netconn_write( conn, data_Gadgets_html, sizeof( data_Gadgets_html), NETCONN_COPY );
						 ...
						 else if( !strncmp( buf, "GET /HyperTerminal", 18) )
            	        	 {
            	        		 			...
            	        		 			"<form name=\"input\" method=\"get\">"
												...
												"<p style=\"text-align: center; font-size:22px; color:#333333;font-weight: bold; \">Отправка сообщения в терминал</p>"
												"<p style=\"text-align: center;font-size: 18px; color:  #003399;\">Введите сообщение для отправки в терминал:"
